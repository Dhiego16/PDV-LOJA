...
import type { Product, CartItem, Sale, SuspendedSale, PaymentMethod, AppSettings } from './types';
import { INITIAL_PRODUCTS, DEFAULT_SETTINGS } from './constants';
import { Search, ShoppingCart, Settings, Moon, Sun, Trash2, Plus, Archive, Package, BarChart2, Clock, History } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const finalizeSale = () => {
    ...
    // Store last sale for Success Modal (preserve change calc)
    const finalChange = paymentMethod === 'money' ? Math.max(0, (parseFloat(cashReceived) || 0) - total) : 0;
    setLastSale({ sale: newSale, change: finalChange });
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white dark:bg-dark-surface border-b border-gray-200 dark:border-dark-border px-6 py-4 flex justify-between items-center shadow-sm z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="secondary" size="sm" onClick={() => setIsHistoryOpen(true)} title="Histórico de Vendas">
              <History size={18} />
            </Button>
            ...
            <Button variant="secondary" size="sm" onClick={() => { setEditingProduct(undefined); setIsProductModalOpen(true); }} title="Novo Produto (F3)">
             ...
             <Button variant="secondary" size="sm" onClick={() => setIsSettingsOpen(true)} title="Configurações">
              <Settings size={18} />
            </Button>
